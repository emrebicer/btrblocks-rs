...
fn main() {
    ...
    unsafe {
        let int_vec = btrblocks::new_intvector(&generate_data(640000, (1 << 12) - 1, 40, 69));
        btrblocks::relation_add_column_int(relation, &"ints".to_string(), int_vec);

        let double_vec = btrblocks::new_doublevector(&generate_data(640000, (1 << 12) - 1, 40, 42));
...
mod btrblocks {
    ...
    unsafe extern "C++" {
        ...
        fn new_intvector(vec: &Vec<i32>) -> *mut IntVector;
        fn new_doublevector(vec: &Vec<f64>) -> *mut DoubleVector;

        unsafe fn relation_add_column_int(
            relation: *mut Relation,
            column_name: &String,
            btr_vec: *mut IntVector,
        );
        unsafe fn relation_add_column_double(
            relation: *mut Relation,
            column_name: &String,
            btr_vec: *mut DoubleVector,
        );
